...
import { getSaasAccountById } from 'apiSdk/saas-accounts';
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Saas Account Detail View
          </Text>
          {hasAccess('saas_account', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/saas-accounts/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              ...
              {hasAccess('onboarding_workflow', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/onboarding-workflows/view/${data?.onboarding_workflow?.id}`}>
                      {data?.onboarding_workflow?.name}
                    </Link>
                  </Text>
                </Flex>
              )}
